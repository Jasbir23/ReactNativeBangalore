...
export default class ScrollSwagger extends Component {
  ...
  dragEnd(e) {
    if (
      e.nativeEvent.contentOffset.y < this.state.selected * 320 - 70 &&
      this.state.selected !== 0
    ) {
      this.refs.scrollRef._component.scrollTo({
        y: (this.state.selected - 1) * 320,
        animated: true
      });
      this.props.shiver();
      this.setState({
      ...
      e.nativeEvent.contentOffset.y > this.state.selected * 320 + 70 &&
      this.state.selected !== this.state.noOfItems - 2
    ) {
      this.refs.scrollRef._component.scrollTo({
        y: (this.state.selected + 1) * 320,
        animated: true
      ...
      <View style={{ flex: 1 }}>
        ...
        >
          <Animated.ScrollView
            ...
            ref="scrollRef"
            onScrollEndDrag={e => {
              this.props.stick ? this.dragEnd(e) : null;
            }}
            style={{ flex: 1, top: 0 }}
